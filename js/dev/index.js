...
document.addEventListener("DOMContentLoaded", () => {
  const supabaseUrl = "https://fcywbhrjsnafjrwtfdwg.supabase.co";
  const supabaseAnonKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZjeXdiaHJqc25hZmpyd3RmZHdnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDkxNDcwMzIsImV4cCI6MjA2NDcyMzAzMn0.qQE93etw715_cZ76e06sJYx0qOokh8A-H3CU5XosIlk";
  const supabase = createClient(supabaseUrl, supabaseAnonKey);
  const classFilter = "2";
  async function fetchProducts() {
    const { data, error } = await supabase.from("Products").select("*").eq("category", classFilter);
    if (error) {
  ...
  document.getElementById("signup-btn").onclick = async () => {
    ...
    if (error) {
      ...
      message.textContent = `Користувач зареєстрований: ${user.email}`;
    } else {
      message.textContent = "Реєстрація успішна, перевірте email для підтвердження.";
    }
  };
  document.getElementById("signin-btn").onclick = async () => {
    ...
    if (error) {
      if (error.message.includes("Email not confirmed")) {
        message.textContent = "Будь ласка, підтвердіть ваш email, перш ніж увійти.";
      } else {
        message.textContent = `Помилка входу: ${error.message}`;
      }
    } else {
      message.textContent = `Користувач увійшов: ${data.user.email}`;
    }
  };
  document.getElementById("signout-btn").onclick = async () => {
    const { error } = await supabase.auth.signOut();
    if (error) {
      message.textContent = `Помилка виходу: ${error.message}`;
    } else {
      message.textContent = `Користувач вийшов`;
    }
  };
  fetchProducts();
});
